...
async function reload(){
    try{
    message=await axios.get("http://localhost:3001/reload")
    let d=message.data.message.split("[][][[]]");
    let c=message.data.completed.split("[][][[]]");
    ...
    if (label) {
        ...
        if (truValue !== '') {
            try {
                ...
                const responseComplete = await axios.post("http://localhost:3001/complete", 
                    {
                    value: truValue
                });
                ...
                const responseDelete = await axios.delete(`http://localhost:3001/delete`,{
                        data:
                        {
                            value:truValue
    ...
    for (const checkbox of selectedBoxes) { // Use a for...of loop for sequential processing
        ...
        if (label) {
            ...
            if (truValue !== '') {
                try {
                    console.log("Posting completion for:", truValue);
                    const responseComplete = await axios.post("http://localhost:3001/add", 
                        {
                        value: truValue
